...
void lpm_insert(iplookup_t *iplookup, uint32_t ip, uint32_t len,
                uint16_t gate) {
        ...
        for i in 25..RAW_SIZE {
            for (k, v) in &self.raw_entries[i] {
                ...
                if (t24entry & OVERFLOW_MASK) == 0 {
                    ...
                    let start = ctlb + (addr & 0xff); // Look at last 8 bits
   (since first 24 are predetermined. let end = start + (1 << (32 - i)); for j
   in ctlb..(ctlb + 256) { if j < start || j >= end { self.tbl_long[j] =
   t24entry; } else { self.tbl_long[j] = *v;
                        }
                    }
                    self.tbl24[addr >> 8] = ((ctlb >> 8) as u16) |
   OVERFLOW_MASK; self.current_tbl_long += 256; } else { let start = (((t24entry
   & (!OVERFLOW_MASK)) as usize) << 8) + (addr & 0xff); let end = start + (1 <<
   (32 - i)); for j in start..end { self.tbl_long[j] = *v;
                    }
                }
            }
        }
  ...
  for (i = 25; i < RAW_SIZE; i++) {
    ...
    for (j = 0; j < cur_num; j++) {
      ...
      if ((t24entry & OVERFLOW_MASK) == 0) {
        ...
        uint32_t start =
            ((uint32_t)(t24entry & (!OVERFLOW_MASK)) << 8) + (addr & 0xFF);
        uint32_t end = start + (1 << (32 - i));
        for (pfx = start; pfx < end; pfx++) {
          iplookup->tbl_long[pfx] = v;
        }
...
pub fn lookup_entry(&self, ip: Ipv4Addr) -> u16 {
        ...
        let t24entry = self.tbl24[addr >> 8];
        if (t24entry & OVERFLOW_MASK) > 0 {
            let index = (((t24entry & !OVERFLOW_MASK) as usize) << 8) + (addr &
0xff); self.tbl_long[index] } else { t24entry
        }
    }
*/
