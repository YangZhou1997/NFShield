...
typedef struct {
  ...
  int *lookup_entry;  // length: lookup_entry_num; record the index of backend
                      // in the backend_ip table;
  uint32_t *
      backend_ip;  // length: backend_num; record the ip of each backend server;
...
void maglev_update_backend(maglev_meta_t *mag_ptr, uint32_t *backend_ip,
                           uint8_t *backend_status);
void maglev_populate_lookup_entry(maglev_meta_t *mag_ptr);

int maglev_init_inner(char *table_name, uint32_t lookup_entry_num,
                      uint32_t backend_num, uint32_t *backend_ip,
                      uint8_t *backend_status, maglev_meta_t *mag_ptr) {
  void *block_ptr;
  uint32_t total_size;
...
void maglev_update_backend(maglev_meta_t *mag_ptr, uint32_t *backend_ip,
  ...
  for (i = 0; i < N; i++) {
    if (backend_status[i] != HEALTHY) continue;

    offset = (uint32_t)fnv_64a_buf((void *)&backend_ip[i], sizeof(uint32_t),
                                   FNV1A_64_INIT);
    skip = (uint32_t)fnv_64a_buf((void *)&backend_ip[i], sizeof(uint32_t),
                                 FNV1A_64_INIT);
    offset %= M;
    skip = skip % (M - 1) + 1;
    for (j = 0; j < M; j++) {
