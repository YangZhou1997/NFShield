...
void mem_test(uint8_t *pkt_ptr){
    ...
    uint32_t start = random() % (MEM_TEST_TOTAL_BYTES - MEM_TEST_ACCESS_BYTES_PER_PKT);
    for(uint32_t i = start; i < start + MEM_TEST_ACCESS_BYTES_PER_PKT - sizeof(uint32_t); i += sizeof(uint32_t)){
        *(uint32_t*)(array + i) += 1;
    }
