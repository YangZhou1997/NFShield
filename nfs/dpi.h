...
// #define AC_DUMP
#ifdef AC_DUMP
  // You must use absolute address, otherwise gem5 will show "Page table fault
  // when accessing virtual address 0"
  ...
  while (fgets(rule_buf, 1023, file_rule)) {
    ...
    rule_buf[len - 1] = '\0';
    // printf("%d: %s\n", len, rule_buf);
    acsmAddPattern2(acsm, (unsigned char *)rule_buf, strlen(rule_buf),
                    1 /*no case*/, 0, 0, 0, (void *)NULL, cnt);
    cnt++;
    // if(cnt >= 1024){
    //     break;
    // }
...
void dpi(uint8_t *pkt_ptr) {
  struct ipv4_hdr *iph =
      (struct ipv4_hdr *)(pkt_ptr + sizeof(struct ether_hdr));
  uint16_t pkt_len =
      htons(((struct ipv4_hdr *)iph)->total_length) + sizeof(struct ether_hdr);

  swap_mac_addr(pkt_ptr);

  int cur = 0;
  acsmSearch2(acsm, (unsigned char *)(pkt_ptr + 54), pkt_len - 54, MatchFound,
